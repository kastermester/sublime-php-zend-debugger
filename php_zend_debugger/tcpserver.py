import struct, asyncore, socket, messagehandler, threading
...
class TcpServerConnection(asyncore.dispatcher_with_send):
	...
	def handle_read(self):
		if self.current_value_read != '' or self.current_value_target_size > 0:
			to_read = self.current_value_target_size - len(self.current_value_read)
			read = ''
			try:
			...
			if len(self.current_value_target_size_read_so_far) == 4:
				size, = struct.unpack('!i', self.current_value_target_size_read_so_far)
