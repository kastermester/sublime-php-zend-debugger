...
class DebuggerServer(threading.Thread):
    ...
    def stop_debug(self):
        if self.listener is not None:
            ...
            self.listener.close()
            self.listener = None
        if self.previous_sigint_listener is not None:
...
class DebuggerSession:
    ...
    def remove_breakpoint(self, file, line):
        ...
        self.send(rm, breakpoint_removed)
    ...
    def get_stack_variables(self, callback):
        msg = {
            'id': 'MSG_GET_VAR',
            'var_expression': 'get_defined_vars() + (isset($this) ? array("this" => $this):array())',
            'depth': 3,
            'path': []
        }
        self.send(msg, callback)
