...
from callstackwindow import frame_depth

class PzdLocalsWindow(object):
	def __init__(self):
		global pzd_socket_opened, pzd_on_continue, frame_depth
		self.socket = None
		self.opened = False
		self.view = None
	...
	def fetch_locals(self, depth):
		print depth
		self.socket.get_stack_variables(int(depth), self.process_locals)
