...
class MessageHandler:
	...
	def read_message(value):
		...
		if not (message_id in MessageHandler.messages):
			raise Exception('Could not find message with id: ' + str(message_id))

		msg = MessageHandler.messages[message_id]

		if msg['id'] != 'MSG_ADD_BREAKPOINT':
			result,offset = MessageHandler.read_format(value, msg['format'], 2)
			result['id'] = msg['id']
		else:
			offset = 2
			result = {}
			result['id'] = msg['id']
			result['req_id'], offset = MessageHandler.read_format_string(value, '!i', offset)
			result['type'], offset = MessageHandler.read_format_string(value, '!h', offset)
			result['lifetime'], offset = MessageHandler.read_format_string(value, '!h', offset)
			if result['type'] & 2 == 2:
				result['condition'], offset = MessageHandler.read_format_string(value, 's', offset)
				if result['type'] & 1 != 1:
					return result

			result['file'], offset = MessageHandler.read_format_string(value, 's', offset)
			if result['type'] & 1 == 1:
				result['lineno'], offset = MessageHandler.read_format_string(value, '!i', offset)
		return result
	...
	def read_format_string(value, format, offset):
		if format == 's':
			...
			return MessageHandler.read(value, str(str_length) + 's', offset+4), offset+4+str_length
		else:
			return MessageHandler.read(value, format, offset), offset+struct.calcsize(format)
	...
	def read_format_tuple(value, format, offset):
		...
		for key, value_format in format:
			result[key], offset = MessageHandler.read_format(value, value_format, offset)
	...
	def read_format_array(value, format, offset):
		...
		array_length,offset = MessageHandler.read_format_string(value, '!i', offset)
		result = []
		i = 0
